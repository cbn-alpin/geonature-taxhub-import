#!/bin/bash
# Encoding : UTF-8
# Import texts in TaxHub attributes.
...
function extractArchive() {
    ...
    if [[ -f "${raw_dir}/${th_filename_archive}" ]]; then
        if [[ ! -f "${raw_dir}/${th_filename_text}" ]]; then
            cd "${raw_dir}/"
...
function importTxt() {
    parseCsv "theme" "th"
    executeCopy "theme"
    executeUpgradeScript "theme" "upsert"

    parseCsv "attribut" "a"
    executeCopy "attribut"
    executeUpgradeScript "attribut" "upsert"

    parseCsv "text" "t"
    executeCopy "text"
    executeUpgradeScript "text" "upsert"
}
